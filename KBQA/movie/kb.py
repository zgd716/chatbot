...
def get_data(index,params):
    ...
    if index==0:
        ...
        query = "MATCH (p:Person)-[r:actedin]->(m:Movie) where m.title='{}' RETURN p.name;".format(params[0])  # 演员列表
    elif index==5:
        ...
        query = "match(n:Person)-[:actedin]-(m:Movie) where n.name ='{}' " \
			 "match(g:Genre)-[:is]-(m) where g.name='{}' return distinct  m.title".format(params[0],params[1])  # 某演员演过什么类型电影有哪些
    elif index==7:
        query = "match(n:Person)-[:actedin]->(m:Movie) where n.name='{}' return m.title".format(params[0])  # 某演员演过什么电影
    elif index==8:
        query = "match(n:Person)-[:actedin]-(m:Movie) where n.name ='{}' and m.rating> {} return m.title".format(params[0],params[1])  # 参演评分 大于 x
    elif index==9:
        query = "match(n:Person)-[:actedin]-(m:Movie) where n.name ='{}' and m.rating < {} return m.title;".format(params[0],params[1])  # 参演评分 小于 x
    elif index==10:
        query = "match(n:Person)-[:actedin]-(m:Movie) where n.name ='{}' " \
